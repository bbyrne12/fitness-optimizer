...
- A missed lift is carried: on a lift day the app looks back four days for a lift that was scheduled and did not happen, and moves it into today, with the rest of the week shifting behind it.
- And on any lift day, the lift is whichever of the week's lifts was trained longest ago, today's included. So a pull day traded for a run is still owed even though nothing was missed outright, and a push day three days after the last push becomes legs or pull instead. When the week has gone to plan this changes nothing. The one placement it will not take is legs the day before the long run. A day with a run or a match on it counts as a trade, not a miss; a lift made up since is not owed twice; a day WHOOP never scored is left alone. So resting on Saturday when push was due means push on the next lift day, not legs.
...
- Exercises rotate inside that shape. A movement that has been in three sessions of its kind in a row may be swapped for one the athlete has done before and not lately, on the same rung of the session and for the same muscle, at most two swaps in a session. The first compound holds for six sessions, because that is what progression is measured on. Swapped-in movements are listed in the morning email. Variety is for staying interested; the research has it level with fixed exercises for size and strength, so never sell it as more than that.
